...
The places where we create these blocks have been christened _block-based editors_. Over the last few years, we've seen a cambrian explosion of these editors across the web. Everywhere you look another block-based editor bubbles up from the evolutionary soup, promising to organise and structure your content in neat little blocks.

At last count I found 23 block-editors in active development, and I've certainly missed some. Despite serving a range of audiences and use caes, they've congregated on an impressively consistent set of interface patterns.

Rather than treating documents as strings of styled text, these editors treat it as a stack of types blocks; each with their own formatting conventions and unique affordances:
...
You begin in what looks like a classic document, type the key `/` <Footnote idName={2}>Or rarely `@` if an app is feeling particularly spicy and innovative</Footnote> or click a floating + icon nearby, and a selection menu appears with a set of block choices:
...
You pick a block and start typing content. If you decide you want a different block, you swap it out and the content adapts to fit its new format. Data and display aren't tied to one another in block land. You can flexibly play with multiple ways to show the same data.

Checklists can become blockquotes. Headers can become collapsible toggles. Tables can become kanban boards. <Footnote idName={3}>There are of course sensible limitations to this. Most applications won't let you turn an image into a code block or a video block.</Footnote>
