...
In the book [A Pattern Language](https://en.wikipedia.org/wiki/A_Pattern_Language) Alexander defined 253 architectural patterns for designing cities, towns, and homes. These became the example patterns for writing patterns. Each one is composed of a context, problem, example, and solution.
...
<BasicImage
  src="https://res.cloudinary.com/dxj9qr5gj/image/upload/v1623585337/maggieappleton.com/notes/xanadu/hover-preview-url_shrink_kxvwtp.png"
  width="1000px"
...
Ted's proposal for visible links relied on another key Xanadu pattern: parallel documents arranged side-by-side.

Arranging two windows side-by-side doesn't solve this. We can't select things from from one window and drag them into the next. The two windows don't know about each other and can't interact. Copying and pasting text, images, or objects it is a cumbersome workaround.
