...
In the mid 1980's the world of computer interfaces cracked in two<Footnote idName={1}>Specifically, 1984 when Apple's Macintosh became the first GUI-based computer to gain market traction. Honorable mentions: [Xerox Star](https://en.wikipedia.org/wiki/Xerox_Star) and [Apple Lisa](https://en.wikipedia.org/wiki/Apple_Lisa)</Footnote>, leaving a gaping divide between two opposing paradigms.

<TwoColumn maxWidth="1200px" gridGap="3rem">

<ImageFrame width="700px" height="423px" src="/images/posts/programming-portals/msdos_igqxdj.jpg" showalt alt="MS-DOS CLI interface" sourceTitle="Betawiki" sourceUrl="https://betawiki.net/wiki/MS-DOS_1.25" />
...
<ImageFrame width="700px" height="423px" src="/images/posts/programming-portals/mac_gn7syi.gif" alt="Apple Macintosh GUI" sourceUrl="http://toastytech.com/guis/macos1.html" showalt sourceTitle="Toasty Tech" />
...
**Programmers** are the tiny, elite subsection of the population who control what computers can do.<Footnote idName={3}>To add some nuance to this, there are other parties controlling what the computers can do: VC's doling out cash, C-suite leaders and product managers directing the vision, and designers orchestrating the user experience. But if the programmers refuse to build it, it's not going to exist. They are the final touch point and nexus of control between the power of the computing machines and the rest of the world.</Footnote> They decide what information appears on the dashboard, what actions are avaliable in the menu, and whether you're allowed to delete your data. They stayed behind in the world of CLIs and text editors.
...
<ImageFrame width="1200px" src="/images/posts/programming-portals/vscode1.png" alt="A very unfamiliar interface for most computer users" showalt  />
...
Search inputs are becoming more structured and programmatic. ["Command K" bars](https://dribbble.com/tags/command_k) are popping up in the middle of complex dashboards. And I keep finding tiny portals back into the programmatic realm; windows I'm calling **programming portals**.
...
Programming portals are small, scoped areas within a graphical interface that give users access to command lines and text-based programmaing. They open a little window into the underlying functionality of an interface. One that lets the user view the underlying properties of their environment, and control a specific set of data, objects, or features of the application.
...
<BasicImage margin="0 auto 0.5rem" src="images/posts/programming-portals/demo-portal-1.jpg" width="900px" />

It allows you to right click and inspect any shape on the canvas:

<BasicImage margin="0 auto 0.5rem" src="images/posts/programming-portals/demo-portal-2.jpg" width="900px" />
 ...
 Seeing metadata about objects is great, but having the power to change them is better. All programming portals give users a ways to write simple logic statements that change the interface. Here I've entered some logic into the formula bar to change the styling of the circle:

<BasicImage margin="0 auto 0.5rem" src="images/posts/programming-portals/demo-portal-4.jpg" width="900px" />
...
<BasicImage margin="0 auto 0.5rem" src="images/posts/programming-portals/demo-portal-6.jpg" width="900px" />

Could I have changed the stroke through a set of GUI elements like input boxes and a colour pickers? Sure! But it wouldn't have been conditional on the circle's size.

In this world, if I resize the circle to make it less than 200px, it would lose the gold border. This is the power of programming. We're writing dynamic rules, not absolute properties of a static image. Composing rules with linear syntax is open ended and flexible in a way that's difficult to recreate in visual interface elements.
...
- Run calculations with the avaliable data

It also doesn't give users access to rewrite the actual source code of the application. They simply have access to a limited programatic interface that gives them more control over how it works.
...
<BasicImage margin="1rem auto 2rem" width="900px" src="/images/posts/programming-portals/hypercard-illo.png" alt="A stack of hypercards with links between them"  />

The remarkable thing about Hypercard is it gave users an enourmous range of control and flexibility. It came with a high-level, built-in programming language called [Hypertalk](https://en.wikipedia.org/wiki/HyperTalk) that let users any edit any element on a card.
...
<ImageFrame width="900px" src="/images/posts/programming-portals/hc1_iykxf7.jpg" alt="The scripting interface in Hypercard" sourceUrl="https://wiki.xxiivv.com/site/hypertalk.html" sourceTitle="Devine Lu Linvega" showalt  />
...
The [team behind it](https://batou.xyz/) was heavily influenced by Hypercard and implemented many of the same design patterns. Each element on a Fermat canvas is a progrogrammable object. By right-clicking on it, you can inspect it to see its properties and edit them inline.

<ImageFrame width="1100px" src="/images/posts/programming-portals/fermat1.png" alt="Inspecting a text object in Fermat" showalt  />
...
<ImageFrame width="900px" src="/images/posts/programming-portals/fermat-script.gif" alt="Editing the behaviour of a button in Fermat to change the value of a text box" showalt  />
...
<ImageFrame width="900px" src="/images/posts/programming-portals/inkbase.jpg" alt="The Inkbase prototype" sourceUrl="https://www.youtube.com/watch?v=ifYuvgXZ108" sourceTitle="Ink & Switch" showalt  />
...
There's something about the hand-drawn aesthetic and touch interactions which makes this feel very far away from Serious Coding. There's no pretense this tool is for making commerical apps. It's clearly personal and low stakes. Yet is capable of sophisticated programming logic in the same way Hypercard and Fermat are.

I want more of it. These portals act as entry ways into a very different relationship with the computer. One where you learn to think programmatically – in rules and dynamic interactions and conditional statements, rather than just manipulating a static object in front of you. It's closing that crack in the interface world between restrictive GUI and open-ended CLIs. 
