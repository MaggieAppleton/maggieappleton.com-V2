...
The notoriously walled-off publishing platform [Medium](https://medium.com/) implemented a similar feature on their articles. You can see the single most  highlighted line from other users. But only one. As if no longform writing could possibly contain more than one useful insight. The ones published to Medium certainly don't.
...
I've done a few [[Silent Synchronous Reading Sessions]] with friends where we all open the same article, quietly annotate with Hypothesis for 30 minutes, then debreif on a zoom call. It works quite well!
